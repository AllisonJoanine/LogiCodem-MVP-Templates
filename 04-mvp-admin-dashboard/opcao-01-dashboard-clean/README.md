# Pulse Dashboard - Dashboard Clean

Admin Dashboard para operacoes que precisam acompanhar indicadores e decisoes do dia, com experiencia demonstravel, responsiva e pronta para adaptar em projetos reais da LogiCodem.

## Como rodar

```bash
npm install
npm run dev
```

Para gerar a versao de producao:

```bash
npm run build
npm run preview
```

## O que o MVP faz

- Projeto React + Vite + TypeScript funcional
- Layout responsivo para celular, tablet e desktop
- Dados mockados organizados e editaveis
- Estados de carregamento, vazio, erro e sucesso quando aplicavel
- Cards de KPI
- Filtros, tabela e grafico CSS
- Exportacao e refresh simulados

## Como adaptar para um cliente real

- Troque os dados em `src/data.ts` por chamadas a uma API ou banco de dados.
- Ajuste as cores em `src/data.ts` dentro de `theme`.
- Conecte formularios, login, carrinho ou fluxos operacionais ao backend do cliente.
- Use os componentes e secoes de `src/App.tsx` como base para novas telas.

## Identidade

- Categoria: Admin Dashboard
- Opcao: Dashboard Clean
- Estilo: Editorial claro
- Pacote: `logicodem-04-mvp-admin-dashboard-opcao-01-dashboard-clean`
